...
export default function EditBatchScreen () {
	...
	useEffect(() => {
		analytics.trackScreen('batch_edit')
		void (async () => {
			if (!batchId) {
				return
			}
			const batch = await getBatchById(executor, batchId)
			if (!batch || batch.archivedAt) {
				Alert.alert('Не найдено', 'Упаковка недоступна.')
				router.back()
				return
			}
			...
			setCabinetId(batch.cabinetId)
			setLocationId(batch.storageLocationId)
			setQuantityText(formatQuantity(batch.quantity))
			setUnit(batch.unit)
			const precision = getExpiryPrecision(batch.expiryDate)
			setExpiryPrecision(precision)
			if (precision === 'year-month' && batch.expiryDate) {
				setExpiryYearMonth(batch.expiryDate)
			}
			if (precision === 'date' && batch.expiryDate) {
				setExpiryDate(batch.expiryDate)
			}
			setPurchaseDate(batch.purchaseDate ?? '')
			setOpenedAt(batch.openedAt ?? '')
			setAfterOpeningValue(
				batch.afterOpeningValue !== null
					? formatQuantity(batch.afterOpeningValue)
					: '',
			)
			if (batch.afterOpeningUnit) {
				setAfterOpeningUnit(batch.afterOpeningUnit)
			}
			setNotes(batch.notes ?? '')
		})()
	}, [batchId, executor, seed.household.id])
	...
	async function handleSave () {
		...
		setSaving(true)
		try {
			await updateBatch(executor, batchId, {
				cabinetId,
	...
	function handleArchive () {
		Alert.alert(
			...
			'Она исчезнет из активного остатка лекарства.',
			[
	...
	return (
		<Screen scroll>
			<ScreenTopBar title="Изменить упаковку" />
			<ChipGroup label="Аптечка">
